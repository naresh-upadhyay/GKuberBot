...
RANGE_LOOKBACK = 20
RANGE_MULTIPLIER = 0.9        # relaxed

VOLUME_LOOKBACK = 20
VOLUME_MULTIPLIER = 1.8       # relaxed

VOLUME_CONFIRM_BARS = 2       # NEW
...
print("🚀 Starting Breakout + Volume backtest (v2)...")

for i in range(RANGE_LOOKBACK, len(df)):
    row = df.iloc[i]

    if np.isnan(row["atr"]) or np.isnan(row["avg_range"]) or np.isnan(row["avg_vol"]):
        continue
    ...
    # ================= PHASE 1 — COMPRESSION =================
    compression = row["avg_range"] < df["avg_range"].rolling(50).mean().iloc[i]
    ...
    # ================= PHASE 3 — VOLUME CONFIRM =================
    if pending_breakout:
        if i - breakout_index > VOLUME_CONFIRM_BARS:
            pending_breakout = False
            continue

        if row["v"] > row["avg_vol"] * VOLUME_MULTIPLIER:
            # ENTER TRADE
...
print("\n====== BREAKOUT + VOLUME BACKTEST RESULT (v2) ======")
